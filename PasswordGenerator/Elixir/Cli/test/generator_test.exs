defmodule PasswordGenerator.GeneratorTest do
  ...
  test "generates a password of requested length" do
    assert {:ok, pw} = Generator.generate(16, true, true, true, true)
  ...
  test "rejects non-positive length" do
    assert {:error, "Password length must be at least 1"} =
  ...
  test "defaults to 16 chars with all categories" do
    assert {:ok, pw} = Generator.generate()
    assert String.length(pw) == 16
  end
end