...
class SQLite implements DatabaseAdapter {
    ...
    public function connect(): void {
        $file = getenv('DB_FILE') ?: 'passwords.db';
        $this->pdo = new PDO("sqlite:$file");
        $this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $this->pdo->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);
        $this->pdo->exec("CREATE TABLE IF NOT EXISTS password_entries (id INTEGER PRIMARY KEY AUTOINCREMENT, password TEXT NOT NULL, length INTEGER DEFAULT 16, created_at TEXT DEFAULT (datetime('now')))");
    }

    public function getAll(string $table): array {
        $stmt = $this->pdo->query("SELECT * FROM $table ORDER BY id DESC LIMIT 50");
