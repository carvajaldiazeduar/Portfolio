...
class SQLServer implements DatabaseAdapter {
    ...
    public function connect(): void {
        ...
        $this->pdo = new PDO("sqlsrv:Server=$host,$port;Database=$name", $user, $pass);
        $this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $this->pdo->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC);
        $this->pdo->exec("IF NOT EXISTS (SELECT * FROM sys.tables WHERE name='password_entries') CREATE TABLE password_entries (id INT IDENTITY(1,1) PRIMARY KEY, password NVARCHAR(MAX) NOT NULL, length INT DEFAULT 16, created_at DATETIME DEFAULT GETDATE())");
    }

    public function getAll(string $table): array {
        $stmt = $this->pdo->query("SELECT TOP 50 * FROM $table ORDER BY id DESC");
