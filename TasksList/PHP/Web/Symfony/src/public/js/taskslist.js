        ...
        async function loadTasks() {
            ...
            list.querySelectorAll('.completeBtn').forEach(btn => btn.addEventListener('click', async () => { await fetch('/api/tasks/' + btn.dataset.id + '/complete', { method: 'PUT' }); loadTasks(); }));
            list.querySelectorAll('.deleteBtn').forEach(btn => btn.addEventListener('click', async () => { await fetch('/api/tasks/' + btn.dataset.id, { method: 'DELETE' }); loadTasks(); }));
        }
