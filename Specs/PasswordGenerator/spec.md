...
- Elixir uses Ecto (`PasswordEntry` schema + `src/priv/repo/migrations/`) + the same `CacheAdapter` behaviour (`CacheType` via `CACHE_TYPE`, `RedisCache` via Redix / `LocalCache` as an Agent under the supervisor).
