...
## Purpose
Message inbox (CRUD): list messages, read one, create and delete. Each message has a subject, sender, body and read/unread state.
...
- C# Web uses EF Core (`InboxesDbContext`) instead of `DatabaseAdapter`.
...
- Elixir uses Ecto (`Message` schema + `src/priv/repo/migrations/`) + the same `CacheAdapter` behaviour (`CacheType` via `CACHE_TYPE`, `RedisCache` via Redix / `LocalCache` as an Agent under the supervisor).
...
- `GET /inboxes` → lists messages (300s cache)
