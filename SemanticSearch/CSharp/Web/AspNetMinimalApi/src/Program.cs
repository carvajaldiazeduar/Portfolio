...
app.MapGet("/api/search", (string? q, VectorStoreAdapter vectorStore, CacheAdapter cache) => {
    if (string.IsNullOrWhiteSpace(q))
        return Results.BadRequest(new { error = "Query parameter 'q' is required" });
    var cached = cache.Get($"search:{q}");
    if (cached != null)
        return Results.Ok(JsonSerializer.Deserialize<JsonNode>(cached));
    var dimension = int.Parse(Environment.GetEnvironmentVariable("VECTOR_DIMENSION") ?? "1536");
    var embedding = new float[dimension];
    var results = vectorStore.Search(embedding, 5);
    cache.Set($"search:{q}", JsonSerializer.Serialize(results), 300);
    return Results.Ok(new { query = q, results });
});

app.MapGet("/api/collections", (VectorStoreAdapter vectorStore) => {
    var collections = vectorStore.ListCollections();
    return Results.Ok(new { collections });
});

app.MapDelete("/api/collections/{name}", (string name, VectorStoreAdapter vectorStore, CacheAdapter cache) => {
    vectorStore.DeleteCollection(name);
    cache.Delete("search:results");
    return Results.Ok(new { message = $"Collection '{name}' deleted" });
});
...
public class PgVectorAdapter : VectorStoreAdapter {
    ...
    public override async Task AddDocuments(List<string> documents, List<float[]> embeddings, List<Dictionary<string, object>> metadata) {
        ...
        await using var conn = await _ds!.OpenConnectionAsync();
        await using var cmd = new NpgsqlCommand($"CREATE TABLE IF NOT EXISTS {table} (id SERIAL PRIMARY KEY, document TEXT, embedding vector, metadata JSONB)", conn);
        await cmd.ExecuteNonQueryAsync();
        foreach (var (doc, emb, meta) in documents.Zip(embeddings).Zip(metadata, (x, m) => (x.First, x.Second, m))) {
            await using var cmd2 = new NpgsqlCommand($"INSERT INTO {table} (document, embedding, metadata) VALUES (@d, @e, @m)", conn);
    ...
    public override async Task<List<SearchResult>> Search(float[] queryEmbedding, int nResults = 5) {
        ...
        var table = $"vector_{collectionName.Replace('-', '_')}";
        await using var conn = await _ds!.OpenConnectionAsync();
        await using var cmd = new NpgsqlCommand($"SELECT document, metadata, embedding <=> @q::vector AS distance FROM {table} ORDER BY embedding <=> @q::vector LIMIT @n", conn);
